...
function AllJobsPageInner() {
  ...
  const [universities, setUniversities] = useState<
    { id: number; name: string; approved: boolean }[]
  >([]);
      ...
      const fetchUniversities = async () => {
        ...
        try {
          ...
          const response = await fetch(
            `/api/employer/universities?orgId=${orgId}`,
            {
              headers: { Authorization: `Bearer ${token}` },
            }
          );
          if (response.ok) {
            ...
            const approvedUniversities = data.filter(
              (uni: any) => uni.approved
            );
  ...
  const totals = useMemo(() => {
    ...
    const totalHires = jobs.reduce(
      (s, j) => s + j.applicationStats.hired,
      0
    );
    const totalApplied = jobs.reduce(
      (s, j) => s + j.applicationStats.applied,
      0
    );
    ...
    return (
      <div className="grid gap-1.5 grid-cols-9 overflow-x-auto">
        {statusConfig.map(({ key, label }) => (
          <div
            key={key}
            className="text-center min-w-[60px] flex-shrink-0"
          >
                        ...
                        <div className="flex flex-wrap bg-gray-100 rounded-lg p-1">
                          {(
                            [
                              ...
                              [
                                "archived",
                                countsByStatus.archived,
                                "Archived",
                              ],
                            ] as [StatusFilter, number, string][]
                          ).map(([key, count, label]) => (
                            <button
                              key={key}
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(v: SeniorityFilter) =>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          setForm({ ...form, visibility: v as any })
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                  ...
                  {(form.visibility === "institutions" ||
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mb-3">
                        Choose which universities can see this job
                        posting.
                      </p>
                      ...
                      {loadingUniversities ? (
                          ...
                          {universities.map((university) => (
                            ...
                            >
                              <Checkbox
                                ...
                                checked={form.universityIds.includes(
                                  university.id
                                )}
                        ...
                        <div className="p-4 text-center text-sm text-muted-foreground border border-[#d4d4d8] rounded-md">
                          No approved universities available. Please request
                          access to universities in your organization
                          settings first.
                        </div>
                      )}
                    </div>
            ...
            {jobs.length === 0 ? (
              ...
              <div className="text-center py-16 bg-white rounded-lg shadow-sm">
                ...
                <p className="text-sm text-gray-500 mb-6">
                  Once you do, they will sit right here for you
                </p>
                <Button
                  onClick={() =>
