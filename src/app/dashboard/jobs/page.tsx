...
import { Briefcase, Plus, Sparkles, Loader2, ArrowLeft, User, Search, HelpCircle, UserPlus, LogOut, Bell, MoreHorizontal, Trash2, Edit } from "lucide-react";
        ...
        const jobsWithStats = await Promise.all(
          data.map(async (job: Job) => {
            try {
              ...
              // Get creator name (simplified - in real app you'd join with users table)
      ...
      <aside className="w-64 bg-[#FEFEFA] border-r border-gray-200 flex flex-col h-screen sticky top-0">
        <div className="p-6">
          ...
          <nav className="space-y-1">
            ...
            <Button variant="ghost" className="w-full justify-start text-gray-700 bg-[#F5F1E8] text-gray-900">
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <Link href={`/dashboard/jobs/${job.id}`} className="flex-1 cursor-pointer">
                        ...
                        <div className="text-xs text-gray-500 mt-1">
                          {job.totalCandidates} candidates • {job.createdBy} • {new Date(job.createdAt).toLocaleDateString()}
                        </div>
                      </Link>
